...
def uri_b64decode(s):
  return urlsafe_b64decode(s + '=' * (lambda x: x % 4 and (4 - x % 4) or 0)(len(s)))#s + '=' * (4 - len(s) % 4))
...
class EpwingDictionary(object):
  SEARCH_OPTIONS = []
  
  uri_base = '/' #e.g. /dict/
  uri_templates = {'entry': 'book/{book_id}/subbook/{subbook_id}/entry/{entry_id}',
  ...
  _book_cache = {}
  #_book_groups = {} #key is the group ID
                    #with epwing, the ID corresponds to a book, which is a collection of subbooks
                    #the format of each group value is: {'name': 'group_name', 'path': '$book_directory/path/to/folder' 'books': [list_of_books]}
  
  
  def __init__(self, book_id):#, URI_prefix=''): #books_directory=self.books_directory
    self.book_id = book_id
    if not self._book_cache.has_key(book_id):
      self._refresh_book_cache()
    self.book_path = self.books_directory + self._book_cache[book_id]['path']
  ...
  @classmethod
  def _books(cls, _book_list={}, _directory=None):
    if not _directory:
      _directory = cls.books_directory
    for item in glob(path.join(_directory, '*')):
      if path.isdir(item) or path.islink(item):
        if path.exists(path.join(item, 'CATALOGS')) or path.exists(path.join(item, 'CATALOG')):
          key = uri_b64encode(path.basename(item))
          if not _book_list.has_key(key):#skip this dictionary if this folder name already exists in loaded books - the danger here is that it might not always skip the same book
            _book_list.update({ key: { 'name': path.basename(item).title(), 'path': path.relpath(item, cls.books_directory) } })
      else:
        _book_list.update(cls._books(_book_list, _directory=item))
    return _book_list
  
  
  @classmethod
  def _refresh_book_cache(cls):
    cls._book_cache = cls._books()
  
  
  @classmethod
  def books(cls):
    'Returns a list of book URIs, IDs, and names'
    if cls._book_cache:
      book_cache = cls._book_cache
    else:
      cls._refresh_book_cache()
      book_cache = cls._book_cache
    ret = []
    for key, val in book_cache.items():
      ret.append({ 'name': val['name'],
                   'id': key,
                   'uri': cls.uri_base + cls.uri_templates['book'].format(book_id=key) }) #'{0}{1}/'.format(self.URI_prefix, key) } )
    return ret

  
  #TODO
  #@classmethod
  #def subbooks(cls, book, URI_prefix=''):
  #  pass
  ...
  def subbooks(self):
    ...
    for subbook in eb_subbook_list(self.book):
      ret.append( { 'name': eb_subbook_title2(self.book, subbook).decode('euc-jp'),
                    ...
                    'id': str(subbook),
                    'uri': self.uri_base + self.uri_templates['subbook'].format(book_id=self.book_id, subbook_id=str(subbook)) }) #'{0}{1}/'.format(self.URI_prefix, str(subbook)) } )
    return ret
  ...
  @classmethod
  def search_all_books(cls, query, search_method='exact', search_options=None, container=None):
    for book in cls.books():
      search_book = cls(book['uri'])
      yield search_book.search(query, search_method=search_method, search_options=search_options, container=container)
    ...
    while True:
      ...
      for heading_location, text_location in hits:
        ...
        if string.strip(content):
          ...
          uri = self.uri_base + self.uri_templates['entry'].format(book_id=self.book_id, subbook_id=str(subbook_id), entry_id=entry_id) #'{0}{1}/'.format(self.uri_base, entry_id)
  ...
  #hooks
  #TODO 'horsey' in chujiten is messed up, see ebview for correct one
  ...
  def _hook_wave(self, book, appendix, container, code, argv):
    if code == EB_HOOK_BEGIN_WAVE:
      ...
      page = int(argv[2])
      offset = int(argv[3])
      audio_id = _positions_to_resource_id([page, offset, data_size])
